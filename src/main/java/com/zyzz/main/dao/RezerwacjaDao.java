...
public interface RezerwacjaDao extends JpaRepository<Rezerwacja, Integer> {
    ...
    boolean czyJestKolizja(@Param("start")LocalDate start, @Param("end") LocalDate end, @Param("id") int id);
    ...
    @Query(value = "SELECT CASE WHEN EXISTS" +
            ...
            "ELSE 'false'" +
            "END", nativeQuery = true)
    boolean czyJestKolizjaDlaRezerwacji(@Param("start")LocalDate start, @Param("end") LocalDate end, @Param("obiektId") int obiektId, @Param("id") int id);
